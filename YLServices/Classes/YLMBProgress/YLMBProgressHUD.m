...
#define YLMBMainThreadAssert() NSAssert([NSThread isMainThread], @"YLMBProgressHUD needs to be accessed on the main thread.");

CGFloat const YLMBProgressMaxOffset = 1000000.f;

static const CGFloat YLMBDefaultPadding = 4.f;
static const CGFloat YLMBDefaultLabelFontSize = 16.f;
static const CGFloat YLMBDefaultDetailsLabelFontSize = 12.f;


@interface YLMBProgressHUD ()
...
@interface YLMBProgressHUDRoundedButton : UIButton
@end
...
+ (instancetype)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    YLMBProgressHUD *hud = [[self alloc] initWithView:view];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
    YLMBProgressHUD *hud = [self HUDForView:view];
    if (hud != nil) {
        hud.removeFromSuperViewOnHide = YES;
...
+ (YLMBProgressHUD *)HUDForView:(UIView *)view {
    NSEnumerator *subviewsEnum = [view.subviews reverseObjectEnumerator];
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
            YLMBProgressHUD *hud = (YLMBProgressHUD *)subview;
...
- (void)commonInit {
    ...
    _animationType = YLMBProgressHUDAnimationFade;
    _mode = YLMBProgressHUDModeIndeterminate;
    _margin = 20.0f;
...
- (void)animateIn:(BOOL)animatingIn withType:(YLMBProgressHUDAnimation)type completion:(void(^)(BOOL finished))completion {
    // Automatically determine the correct zoom animation type
    if (type == YLMBProgressHUDAnimationZoom) {
        type = animatingIn ? YLMBProgressHUDAnimationZoomIn : YLMBProgressHUDAnimationZoomOut;
    }
    ...
    if (animatingIn && bezelView.alpha == 0.f && type == YLMBProgressHUDAnimationZoomIn) {
        bezelView.transform = small;
    } else if (animatingIn && bezelView.alpha == 0.f && type == YLMBProgressHUDAnimationZoomOut) {
    ...
    dispatch_block_t animations = ^{
        ...
        } else if (!animatingIn && type == YLMBProgressHUDAnimationZoomIn) {
            bezelView.transform = large;
        } else if (!animatingIn && type == YLMBProgressHUDAnimationZoomOut) {
            bezelView.transform = small;
        }
...
- (void)done {
    ...
    YLMBProgressHUDCompletionBlock completionBlock = self.completionBlock;
    if (completionBlock) {
        completionBlock();
    }
    id<YLMBProgressHUDDelegate> delegate = self.delegate;
...
- (void)setupViews {
    ...
    YLMBBackgroundView *backgroundView = [[YLMBBackgroundView alloc] initWithFrame:self.bounds];
    backgroundView.style = YLMBProgressHUDBackgroundStyleSolidColor;
    ...
    YLMBBackgroundView *bezelView = [YLMBBackgroundView new];
    ...
    label.font = [UIFont boldSystemFontOfSize:YLMBDefaultLabelFontSize];
    label.opaque = NO;
    ...
    detailsLabel.font = [UIFont boldSystemFontOfSize:YLMBDefaultDetailsLabelFontSize];
    detailsLabel.opaque = NO;
    detailsLabel.backgroundColor = [UIColor clearColor];
    _detailsLabel = detailsLabel;

    UIButton *button = [YLMBProgressHUDRoundedButton buttonWithType:UIButtonTypeCustom];
    button.titleLabel.textAlignment = NSTextAlignmentCenter;
    button.titleLabel.font = [UIFont boldSystemFontOfSize:YLMBDefaultDetailsLabelFontSize];
...
- (void)updateIndicators {
    ...
    BOOL isRoundIndicator = [indicator isKindOfClass:[YLMBRoundProgressView class]];

    YLMBProgressHUDMode mode = self.mode;
    if (mode == YLMBProgressHUDModeIndeterminate) {
    ...
    else if (mode == YLMBProgressHUDModeDeterminateHorizontalBar) {
        // Update to bar determinate indicator
        [indicator removeFromSuperview];
        indicator = [[YLMBBarProgressView alloc] init];
        [self.bezelView addSubview:indicator];
    }
    else if (mode == YLMBProgressHUDModeDeterminate || mode == YLMBProgressHUDModeAnnularDeterminate) {
        if (!isRoundIndicator) {
            // Update to determinante indicator
            [indicator removeFromSuperview];
            indicator = [[YLMBRoundProgressView alloc] init];
            [self.bezelView addSubview:indicator];
        }
        if (mode == YLMBProgressHUDModeAnnularDeterminate) {
            [(YLMBRoundProgressView *)indicator setAnnular:YES];
        }
    } 
    else if (mode == YLMBProgressHUDModeCustomView && self.customView != indicator) {
        // Update custom view indicator
        [indicator removeFromSuperview];
        indicator = self.customView;
        [self.bezelView addSubview:indicator];
    }
    else if (mode == YLMBProgressHUDModeText) {
        [indicator removeFromSuperview];
        indicator = nil;
    }
...
- (void)updateViewsForColor:(UIColor *)color {
    ...
    if ([indicator isKindOfClass:[UIActivityIndicatorView class]]) {
        ...
        appearance = [UIActivityIndicatorView appearanceWhenContainedIn:[YLMBProgressHUD class], nil];
#else
        // For iOS 9+
        appearance = [UIActivityIndicatorView appearanceWhenContainedInInstancesOfClasses:@[[YLMBProgressHUD class]]];
#endif
        
        if (appearance.color == nil) {
            ((UIActivityIndicatorView *)indicator).color = color;
        }
    } else if ([indicator isKindOfClass:[YLMBRoundProgressView class]]) {
        YLMBRoundProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [YLMBRoundProgressView appearanceWhenContainedIn:[YLMBProgressHUD class], nil];
#else
        appearance = [YLMBRoundProgressView appearanceWhenContainedInInstancesOfClasses:@[[YLMBProgressHUD class]]];
#endif
        if (appearance.progressTintColor == nil) {
            ((YLMBRoundProgressView *)indicator).progressTintColor = color;
        }
        if (appearance.backgroundTintColor == nil) {
            ((YLMBRoundProgressView *)indicator).backgroundTintColor = [color colorWithAlphaComponent:0.1];
        }
    } else if ([indicator isKindOfClass:[YLMBBarProgressView class]]) {
        YLMBBarProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [YLMBBarProgressView appearanceWhenContainedIn:[YLMBProgressHUD class], nil];
#else
        appearance = [YLMBBarProgressView appearanceWhenContainedInInstancesOfClasses:@[[YLMBProgressHUD class]]];
#endif
        if (appearance.progressColor == nil) {
            ((YLMBBarProgressView *)indicator).progressColor = color;
        }
        if (appearance.lineColor == nil) {
            ((YLMBBarProgressView *)indicator).lineColor = color;
        }
    } else {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
...
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
    YLMBBackgroundView *bezelView = self.bezelView;
...
- (void)updatePaddingConstraints {
    ...
    [self.paddingConstraints enumerateObjectsUsingBlock:^(NSLayoutConstraint *padding, NSUInteger idx, BOOL *stop) {
        ...
        padding.constant = (firstVisible && (secondVisible || hasVisibleAncestors)) ? YLMBDefaultPadding : 0.f;
...
- (void)setMode:(YLMBProgressHUDMode)mode {
    if (mode != _mode) {
        _mode = mode;
        [self updateIndicators];
    }
...
- (void)setCustomView:(UIView *)customView {
    if (customView != _customView) {
        ...
        if (self.mode == YLMBProgressHUDModeCustomView) {
            [self updateIndicators];
        }
    }
}
...
@implementation YLMBRoundProgressView
...
@implementation YLMBBarProgressView
...
@implementation YLMBBackgroundView
...
- (instancetype)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_7_0) {
            _style = YLMBProgressHUDBackgroundStyleBlur;
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000 || TARGET_OS_TV
            ...
            _style = YLMBProgressHUDBackgroundStyleSolidColor;
...
- (void)setStyle:(YLMBProgressHUDBackgroundStyle)style {
    if (style == YLMBProgressHUDBackgroundStyleBlur && kCFCoreFoundationVersionNumber < kCFCoreFoundationVersionNumber_iOS_7_0) {
        style = YLMBProgressHUDBackgroundStyleSolidColor;
    }
    if (_style != style) {
        _style = style;
        [self updateForBackgroundStyle];
...
- (void)updateForBackgroundStyle {
    YLMBProgressHUDBackgroundStyle style = self.style;
    if (style == YLMBProgressHUDBackgroundStyleBlur) {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000 || TARGET_OS_TV
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
...
- (void)updateViewsForColor:(UIColor *)color {
    if (self.style == YLMBProgressHUDBackgroundStyleBlur) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
            self.backgroundColor = self.color;
        } else {
#if !TARGET_OS_TV
...
@implementation YLMBProgressHUDRoundedButton
