...
@interface KNavigationBarManager ()
...
@implementation KNavigationBarManager
...
+ (void)changeAlphaWithCurrentOffset:(CGFloat)currentOffset {
    KNavigationBarManager *manager = [self sharedManager];
...
+ (KNavigationBarManager *)sharedManager {
    static KNavigationBarManager *manager;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        manager = [[KNavigationBarManager alloc] init];
        [self initBaseData:manager];
    });
    return manager;
}

+ (void)initBaseData:(KNavigationBarManager *)manager {
